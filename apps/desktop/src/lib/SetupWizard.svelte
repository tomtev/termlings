<script lang="ts">
  ...
  const installCommands: Record<string, { label: string; command: string }> = {
    termlings: { label: 'Termlings CLI', command: 'npm install -g termlings@latest' },
...
<div class="setup-wizard">
  {#if view === 'check'}
    <div class="check-view">
      <div class="header">
        <h1>Setup</h1>
        <p>termlings needs a few tools to work.</p>
      </div>
      ...
      {:else if report}
        <div class="deps-list">
          <div class="dep-item" class:installed={report.termlings.installed}>
            <div class="dep-info">
              <span class="dep-name">Termlings CLI</span>
            ...
            {#if !report.termlings.installed}
              <button class="install-btn" onclick={() => startInstall('termlings')}>Install</button>
            {/if}
          </div>
        ...
        <div class="actions">
          <button
            class="continue-btn"
            disabled={!report.termlings.installed}
            onclick={onComplete}
          >
            {report.termlings.installed ? 'Continue' : 'Install Termlings CLI to continue'}
          </button>
        </div>
      {/if}
    </div>
