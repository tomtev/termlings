<script lang="ts">
  ...
  import WorkspacePanel from './settings/WorkspacePanel.svelte';

  export type SettingsTab = 'general' | 'workspace' | 'appearance';
  ...
  <div class="modal" data-modal data-sidebar-layout>
    ...
    <main>
      <div class="modal-header">
        <h3>{{ general: 'General', workspace: 'Workspace', appearance: 'Appearance' }[activeTab]}</h3>
...
<style>
  .modal-backdrop {
    position: fixed;
    inset: 0;
    background: rgb(from var(--foreground) r g b / 0.45);
    display: flex;
    align-items: center;
