...
/// Lightweight HTTP server for future runtime/app notifications.
...
impl HookServer {
    pub fn start(app: AppHandle) -> Result<Self, String> {
        ...
        // Write app.port so local integrations can discover the app.
        if let Some(home) = dirs::home_dir() {
            let port_path = home.join(".termlings-app").join("app.port");
